...
class RigidBodyDynamics:
    ...
    def derivatives(
        ...
        acceleration_ned = total_force_ned / self.params.mass + env_gravity
        ...
        omega_dot = self.inv_inertia @ (total_moment_body - np.cross(omega, self.params.inertia @ omega))
    ...
    def integrate(self, state: SixDofState, derivatives: tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], dt: float) -> SixDofState:
        ...
        next_state.quaternion_bn = next_state.quaternion_bn / np.linalg.norm(next_state.quaternion_bn)
    ...
    def rk4_step(
        ...
        def derivatives(local_state: SixDofState):
            return self.derivatives(local_state, thrust, moment_body, env_gravity, wind_ned, external_force_body, external_moment_body)

        k1 = self.derivatives(state, thrust, moment_body, env_gravity, wind_ned, external_force_body, external_moment_body)
        ...
        k2 = self.derivatives(state_k2, thrust, moment_body, env_gravity, wind_ned, external_force_body, external_moment_body)
        ...
        k3 = self.derivatives(state_k3, thrust, moment_body, env_gravity, wind_ned, external_force_body, external_moment_body)
        ...
        k4 = self.derivatives(state_k4, thrust, moment_body, env_gravity, wind_ned, external_force_body, external_moment_body)
        ...
        next_state.angular_velocity_body += dt / 6.0 * (k1[3] + 2 * k2[3] + 2 * k3[3] + k4[3])
        return next_state
