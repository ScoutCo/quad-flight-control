"""Unit tests for the simplified simulator dynamics."""
...
def test_constant_velocity_command_reached() -> None:
    config = SimpleSimulatorConfig(dt=0.02)
...
def test_simple_telemetry_logger_records_data(tmp_path, ref_velocity) -> None:
    config = SimpleSimulatorConfig(dt=0.05)
    sim = SimpleSimulator(config)
    log_path = tmp_path / 'simple_log.csv'
    ...
    expected_headers = SimpleTelemetryLogger.HEADERS
